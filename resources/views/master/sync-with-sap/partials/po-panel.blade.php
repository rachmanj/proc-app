            ...
            <div class="card-body">
                ...
                @if ($lastSync)
                    <div class="alert alert-info">
                        ...
                        <br>
                        <strong>Date Range:</strong> {{ $lastSync->start_date->format('d M Y') }} -
                        {{ $lastSync->end_date->format('d M Y') }}
                        <br>
                        <strong>Results:</strong> Synced: {{ $lastSync->records_synced }}, Created:
                        {{ $lastSync->records_created }}, Skipped: {{ $lastSync->records_skipped }}
                        @if ($lastSync->convert_status === 'failed')
                            <br><span class="text-danger"><strong>Conversion Failed:</strong>
                                {{ $lastSync->error_message }}</span>
                        @endif
                    </div>
                @else
    ...
    <script>
        $(document).ready(function() {
            ...
            function initPoPanel() {
                ...
                // TODAY button
                $('#po-today-btn').off('click').on('click', function() {
                    const today = getTodayUTC8();
                    const dateStr = formatDateForInput(today);
                    $('#po-selected-start-date').val(dateStr);
                    $('#po-selected-end-date').val(dateStr);
                    $('#po-custom-date-range').css('display', 'none');
                    $('#po-custom-btn').removeClass('active');
                    $(this).addClass('active').siblings().removeClass('active');
                });

                // YESTERDAY button
                $('#po-yesterday-btn').off('click').on('click', function() {
                ...
                // CUSTOM button
                $('#po-custom-btn').off('click').on('click', function() {
                    const $dateRange = $('#po-custom-date-range');
                    const $btn = $(this);
                    ...
                    if (currentDisplay === 'none' || !currentDisplay || currentDisplay === '') {
                        ...
                        $btn.addClass('active');
                        $('#po-today-btn, #po-yesterday-btn').removeClass('active');
                    } else {
                ...
                // Custom date inputs
                $('#po-start-date, #po-end-date').off('change').on('change', function() {
                ...
                // Sync button
                $('#po-sync-btn').off('click').on('click', function() {
                ...
                // Truncate button
                $('#po-truncate-btn').off('click').on('click', function() {
                    Swal.fire({
                        title: 'Are you sure?',
            ...
            // Initialize when PO tab is shown
            $('#po-tab').on('shown.bs.tab', function() {
                initPoPanel();
            });

            // Also initialize if PO tab is already active
            if ($('#po-tab').hasClass('active')) {
                initPoPanel();
            } else {
                // Wait a bit and check again
                setTimeout(function() {
                    if ($('#po-panel').hasClass('active') || $('#po-panel').hasClass('show')) {
                        initPoPanel();
                    }
                }, 100);
            }
